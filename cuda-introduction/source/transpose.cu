...
int main(int argc, char *argv[])
{
    ...
    std::cout << "***Device To Device Copy***" << std::endl;
    {
        ...
        CUDA(cudaMemcpy(b, d_b, sizeX * sizeY * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    std::cout << "***Naive Transpose***" << std::endl;
    {
        ...
        matrixTransposeNaive << <dims.dimBlock, dims.dimGrid >> > (d_a, d_b, sizeX, sizeY);
